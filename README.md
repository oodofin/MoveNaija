...
The five Blue Line stations and both travel directions are sourced from [LAMATA's Blue Line schedule](https://www.lamata-ng.com/blue-line-train-schedule/). The approximately 18-minute end-to-end time comes from [LAMATA's published schedule table](https://www.lamata-ng.com/our-schedule/). The eight Red Line stations, in both directions, and their approximately 50-minute end-to-end duration come from [LAMATA's Monday–Friday timetable](https://www.lamata-ng.com/our-schedule/red-line/). No rail fare or station coordinates have been seeded because neither has been substantiated from these sources. Check the published operating-day timetable before travel.

[LAGFERRY's published schedule](https://lagferry.gov.ng/routes-schedules/) separately supports both directions of five terminal pairs: Ikorodu–Falomo, Badore–Falomo, Ebute Ojo–CMS/Marina, Mile 2–CMS/Marina and Badore–Ijede. These are seeded with the listed endpoint fares and, where consistent, the published journey durations. Badore–Ijede's printed duration conflicts with some timetable entries, so its duration remains unknown. No coordinates or unlisted intermediate docks are guessed; names identify the documented terminals/jetty. Ferry sailings and prices should be confirmed with the operator, particularly because the schedule page does not state a fare-effective date. The database seed is idempotent and retains admin deactivations. The seven original demonstration stops and six sample journeys remain separate, unverified sample data. Broad Lagos coverage requires verified imports and field checks; a missing route is shown as missing, not fabricated.
...
`GET /api/fares/published?origin=Oshodi&destination=Ajah` searches a bundled snapshot of [LAMATA's published Bus Reform Initiative fare list](https://www.lamata-ng.com/bus-fare/), labelled effective 1 March 2026. The snapshot has 78 entries, including two conflicting Mile 2–TBS BRT amounts. Those are shown as conflicting instead of selecting one. It matches explicitly named endpoint pairs; reverse matches are identified as reverse listings, not confirmed return services. Published endpoint fares do not supply boarding stops, intervening stops, transfers, schedules or exact walking directions. They are **fare references**, never automatically connected journey edges in the verified stop network. A published corridor fare must not be presented as the total price for a different multimode itinerary. Refresh the source snapshot after checking a newer published list, and disclose its date to commuters. Informal danfo and keke fares remain unknown until reviewed reports support them.

When an exact-name search has no verified stop-by-stop journey, the planner also checks the LAMATA fare snapshot for named endpoint services. It can display a **possible service sequence** of up to three listed corridors joined at identical area names, with each published fare and the arithmetic sum of those fares. For example, Ikotun → Ajah has listed Ikotun–Marina/CMS and Marina/CMS–Ajah services. This is a lead for further checking: it does **not** prove an operational transfer, accessible boarding points, a departure time, or an end-to-end price. A reversed listing is never treated as a confirmed return service; ambiguous multi-hyphen labels are excluded. These leads are kept separate from verified `/api/journeys` options and cannot be saved as reviewed journey routes. Journeys without supporting published services still state the specific missing information. Covering Ikorodu → Epe, Badagry → Ikeja, Epe → Lekki, and Ojo → Victoria Island requires additional reviewed transport connections. Geocoding place names and importing mapped stops alone cannot establish those services. Structured, source-backed route datasets can be added through the existing admin import after validation.
